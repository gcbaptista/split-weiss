...
export function calculateValue(total: Decimal | string, inputs: SplitInput[]): SplitResult[] {
  ...
  if (!sum.toDecimalPlaces(2).equals(t.toDecimalPlaces(2))) {
    throw new Error(`Split amounts (${sum.toFixed(2)}) do not equal total (${t.toFixed(2)})`);
  }
  return results;
}
